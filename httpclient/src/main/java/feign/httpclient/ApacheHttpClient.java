...
public final class ApacheHttpClient implements Client {
    ...
    HttpUriRequest toHttpUriRequest(Request request, Request.Options options) throws URISyntaxException {
        ...
        RequestConfig requestConfig =
                (client instanceof Configurable ? RequestConfig.copy(((Configurable) client).getConfig())
                        ...
                        .setConnectTimeout(options.connectTimeoutMillis()) // 10000
                        .setSocketTimeout(options.readTimeoutMillis()) // 60000
                        .setRedirectsEnabled(options.isFollowRedirects()) // true
                        .build();
     ...
     * 主要步骤如下：
     * <p>
     * 获取 Apache HttpClient 的响应状态码和状态短语。
     * 遍历响应头，构建 Feign 的响应头信息。
     * 创建并返回一个包含状态码、状态短语、响应头、原始请求和响应体的 Feign 的 Response 对象。
     * 这个方法的核心是将 Apache HttpClient 的响应信息映射到 Feign 的响应对象，确保传递给 Feign 的信息能够被正确解析和处理。
     *
     * @param httpResponse
     * @param request
     * @return
    ...
    Response toFeignResponse(HttpResponse httpResponse, Request request) throws IOException {
        ...
        // 构建 Feign 的响应头信息: {X-Frame-Options=[DENY], Transfer-Encoding=[chunked], Keep-Alive=[timeout=60], Cache-Control=[no-cache, no-store, max-age=0, must-revalidate], vary=[accept-encoding], X-Content-Type-Options=[nosniff], Connection=[keep-alive], Pragma=[no-cache], Expires=[0], X-XSS-Protection=[1; mode=block], Date=[Sun, 07 Jan 2024 13:51:08 GMT], Content-Type=[application/json]}
